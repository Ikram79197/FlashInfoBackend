...
@Repository
public interface CaMensuelRepository extends JpaRepository<CaNonVie, Long> {
            ...
            FROM (
                SELECT 
                    ...
                    CASE 
                        ...
                        ELSE 'DIVERS' 
                    END AS Code_branche,
                    SUM(CASE WHEN ANNEE = CAST(YEAR(GETDATE()) - 1 AS VARCHAR(4)) THEN prime_nette ELSE 0 END) AS primetotale_N_1,
                    0 AS primetotale_N
                FROM SMP_PROD_BU_BRH_FIN
                WHERE code_branche <> 'XX' 
                    AND NOT (mois_annul BETWEEN '01' AND '12')
                    AND mois = CAST(MONTH(GETDATE()) AS VARCHAR(2))
                    AND VISION = 'C'
